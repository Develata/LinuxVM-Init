...
init_flow() {
  ...
  if confirm '执行步骤1（系统更新）？[y/N]' 'Run step 1 (system update)? [y/N]'; then
    run_step 'system_update' system_update
  else
    record_step 'system_update' 'skipped'
  fi
  ...
  if confirm '执行步骤2（常用工具）？[y/N]' 'Run step 2 (tools)? [y/N]'; then
    run_step 'tools_install' tools_install
  else
    record_step 'tools_install' 'skipped'
  fi
  ...
  if confirm '执行步骤3（创建普通用户）？[y/N]' 'Run step 3 (create user)? [y/N]'; then
    run_step 'user_add' user_add
  else
    record_step 'user_add' 'skipped'
  fi
  ...
  if confirm '执行步骤4（SSH 与防火墙）？[y/N]' 'Run step 4 (SSH and firewall)? [y/N]'; then
  ...
  if confirm '执行步骤5（fail2ban）？[y/N]' 'Run step 5 (fail2ban)? [y/N]'; then
    run_step 'fail2ban_setup' fail2ban_setup
  else
    record_step 'fail2ban_setup' 'skipped'
  fi

  say '步骤6/8：自动安全更新。说明：启用后系统会自动执行安全更新。' 'Step 6/8: unattended upgrades. Note: enables automatic security updates.'
  if confirm '执行步骤6（自动安全更新）？[y/N]' 'Run step 6 (unattended upgrades)? [y/N]'; then
    run_step 'unattended_enable' unattended_enable
  else
    record_step 'unattended_enable' 'skipped'
  fi
