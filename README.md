...
### Init 步骤提示（与面板一致）
- 步骤 1/7：系统更新（更新软件包索引并安装可升级项，可能耗时）。
- 步骤 2/7：常用工具（`vim` 和 `command-not-found` 默认跳过，直接回车即可）。
- 步骤 3/7：创建普通用户（先输入用户名，再设置密码；资料项可连续回车，最后输入 `Y` 确认）。
- 步骤 4/7：SSH 与防火墙（若启用 SSH 设置，只需选一次端口，然后配置防火墙并应用 SSH）。
- 步骤 5/7：fail2ban（默认参数较保守，连续失败登录会触发封禁）。
- 步骤 6/7：自动安全更新（启用后系统会自动执行安全更新）。
- 步骤 7/7：Swap（可选，不需要可直接回车跳过）。
